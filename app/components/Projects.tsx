        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8">
          {projects.map((project) => (
            <button
              ...
              className="group text-left w-full h-96 lg:h-[28rem] rounded-xl border border-neutral-800 overflow-hidden hover:border-neutral-600 transition-colors duration-200 flex flex-col bg-neutral-900"
            >
              <div className="relative w-full h-60 lg:h-72 flex-shrink-0 overflow-hidden">
                <Image
                  src={project.image}
                  alt={project.title}
                  fill
              ...
              <div className="p-5 lg:p-6 flex flex-col flex-grow justify-between pb-6 lg:pb-7">
                <div className="flex-grow">
                  <h3 className="text-xl lg:text-2xl font-bold mb-2 group-hover:text-neutral-200 transition-colors duration-200">
                    {project.title}
                  </h3>
                ...
                <div className="flex gap-2 flex-wrap mt-auto pt-3">
                  {project.technologies.slice(0, 4).map((tech) => (
                    <span
                      key={tech}
                      className="px-3 py-1.5 text-xs font-medium rounded-lg bg-neutral-800 border border-neutral-700 text-neutral-300"
                    >
                      {tech}
                    </span>
                  ))}
                  {project.technologies.length > 4 && (
                    <span className="px-3 py-1.5 text-xs font-medium text-muted-foreground rounded-lg bg-neutral-800 border border-neutral-700">
                      +{project.technologies.length - 4}
                    </span>
                  )}
                </div>
